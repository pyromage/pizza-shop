...
describe("Infinite Loop Detection Tests", () => {
  ...
  test("should not have infinite DOM mutations or loops", async () => {
    ...
    await page.evaluate(() => {
      ...
      const observer = new MutationObserver((mutations) => {
        ...
        if (mutations.length > 10) {
          console.warn(
            "High mutation rate detected:",
            mutations.length,
            "mutations"
          );
          console.warn(
            "Affected elements:",
            mutations.map((m) => m.target.tagName || "unknown").join(", ")
          );
        }
      });
    ...
    console.log(
      `Time since last mutation: ${mutationData.timeSinceLastMutation}ms`
    );
    ...
    const potentialInfiniteLoop =
      mutationData.count > excessiveMutationThreshold &&
      ...
      await page.evaluate(() => {
        ...
        window.setTimeout = function (callback, delay) {
          ...
          if (timeoutCount > 100 && delay < 50) {
            console.warn("Excessive setTimeout calls detected", {
              timeoutCount,
              delay,
            });
  ...
  test("should monitor for memory leaks or excessive resource usage", async () => {
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, config.observationTime / 2)
    );
