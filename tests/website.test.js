...
describe("Millbrook Pizza Website Tests", () => {
  ...
  test("should add items to the cart", async () => {
    ...
    const cartTotal = await page.$eval("#cart-total", (el) =>
  ...
  test("should increase, decrease, and remove items from the cart", async () => {
    ...
    const initialQuantity = await page.$eval(".quantity", (el) =>
      parseInt(el.textContent)
    );
    const initialTotal = await page.$eval("#cart-total", (el) =>
      parseFloat(el.textContent)
    );
    ...
    const increasedQuantity = await page.$eval(".quantity", (el) =>
      parseInt(el.textContent)
    );
    expect(increasedQuantity).toBe(initialQuantity + 1);

    // Check if total increased
    const increasedTotal = await page.$eval("#cart-total", (el) =>
      parseFloat(el.textContent)
    );
    ...
    const decreasedQuantity = await page.$eval(".quantity", (el) =>
      parseInt(el.textContent)
    );
